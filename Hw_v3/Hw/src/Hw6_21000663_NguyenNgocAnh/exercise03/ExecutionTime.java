...
public class ExecutionTime {
    ...
    public static void main(String[] args) {
        try {
            ...
            Double[] heapArray = new Double[80];
            
            //execute time
            for (int index = 0; index < 80; index++) {
                execuTime(arrayIntegers.get(index), N.get(index), index, bubArray, selArray, insArray, quiArray, merArray, heapArray);
                System.out.println("Success..." + index);
            }

            //write file
            try {
                FileWriter writer = new FileWriter("src\\Hw6_21000663_NguyenNgocAnh\\exercise03\\data\\execution_time.txt");

                writer.write("N\tBubble\tSelection\tInsertion\tQuick\tMerge\tHeap\n");
                for (int i = 0; i < 80; i++) {
                    writer.write(N.get(i) + "\t" + bubArray[i] + "\t" + selArray[i] + "\t" + insArray[i] + "\t" + quiArray[i] + "\t" + merArray[i] + "\t" + heapArray[i] + "\n");
                }
                writer.close();
                System.out.println("Success...");
            } catch (IOException e) {
    ...
    //execute time
    public static void execuTime(Integer[] array, Integer n, Integer index, Double[] bubArray, Double[] selArray, Double[] insArray, Double[] quiArray, Double[] merArray, Double[] heapArray) {
        ...
        startTime = System.nanoTime();
        SortAlgorithm.heapSort(array, n);
        endTime = System.nanoTime();
        duration = (endTime - startTime);
        seconds = (double)duration / 1_000_000_000.0;
        heapArray[index] = seconds;
    }
}
