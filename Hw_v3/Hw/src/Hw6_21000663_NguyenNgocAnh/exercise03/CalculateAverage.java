...
public class CalculateAverage {
    public static void main(String[] args) {
        ...
        try {
            File file = new File("src\\Hw6_21000663_NguyenNgocAnh\\exercise03\\data\\execution_time.txt");
            ...
            ArrayList<Double> heaArray = new ArrayList<>();
            ...
            Double[] heaArrayAvg = new Double[4];
            for (int i = 0; i < 4; i++) {
                Double sum = 0.0;
                for (int j = 0; j < 20; j++) {
                    sum += heaArray.get(i * 20 + j);
                }
                heaArrayAvg[i] = sum / 20;
            }
            
            // write file
            try {
                FileWriter writer = new FileWriter("src\\Hw6_21000663_NguyenNgocAnh\\exercise03\\data\\average_execution_time.txt");

                writer.write("N\tBubble\tSelection\tInsertion\tQuick\tMerge\tHeap\n");
                for (int i = 0; i < 4; i++) {
                    writer.write(N.get(i * 20) + "\t" + bubArrayAvg[i] + "\t" + selArrayAvg[i] + "\t" + insArrayAvg[i] + "\t" + quiArrayAvg[i] + "\t" + merArrayAvg[i] + "\t" + heaArrayAvg[i] + "\n");
                }
                writer.close();
                System.out.println("Success...");
            } catch (IOException e) {
